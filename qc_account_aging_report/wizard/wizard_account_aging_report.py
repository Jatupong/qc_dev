        ...
        for move in invoice_ids:
            ...
            if invoice_line_is_downpayment:
                ...
                if move.currency_id != move.company_currency_id:
                    ...
                    down_payment_amount_other_currency = abs(sum(invoice_line_is_downpayment.mapped('price_total')))
                    down_payment_amount = move.currency_id._convert(price_total,
                                                                    move.company_currency_id,
                                                                    move.company_id,
                                                                    move.invoice_date or move.date)
                else:
            ...
            value = {
                ...
                'payment_term': payment_term,
                'length_payment_term': length_payment_term,
                'credit_limit_amount': credit_limit and '{0:,.2f}'.format(credit_limit) or '',
                'due_date': move.invoice_date_due.strftime('%d/%m/%Y'),
                'creditor_age': creditor_age or '',
                'currency_name': currency_name,
                'currency_rate': currency_rate,
                ...
                'balance_amount_other_currency': '{0:,.2f}'.format(balance_amount_other_currency),
                'balance_amount': '{0:,.2f}'.format(balance_amount),
                'credit_limit_note': credit_limit_note and '{0:,.2f}'.format(credit_limit_note) or '',
            }
            record.append(value)
