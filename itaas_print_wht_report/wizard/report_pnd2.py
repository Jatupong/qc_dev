...
class report_report_pnd2(models.AbstractModel):
    ...
    def _get_header_info(self, date_from, date_to, company_id):
        ...
        print('company', type(company_id))
        print('company_vat', company_id.vat)
        return {
            ...
            'company_tumbon': company_id.tumbon,
    ...
    def _get_report_values(self, docids, data=None):
        ...
        tax_info1 = self._get_tax_month(data['date_from'], data['date_to'], company_id, ['ค่าธรรมเนียม ค่านายหน้า ฯลฯ'])
        print('tax_info1',tax_info1)
        tax_info2 = self._get_tax_month(data['date_from'], data['date_to'], company_id, ['ค่าแห่งลิขสิทธิ์ ฯลฯ'])
        print('tax_info2',tax_info2)
        tax_info3 = self._get_tax_month(data['date_from'], data['date_to'], company_id, ['ดอกเบี้ย'])
        print('tax_info3',tax_info3)
        tax_info4 = self._get_tax_month(data['date_from'], data['date_to'], company_id, ['เงินปันผล เงินส่วนแบ่งกำไร ฯลฯ'])
        print('tax_info4',tax_info4)
        tax_info = self._get_tax_month(data['date_from'], data['date_to'], company_id, ['ค่าธรรมเนียม ค่านายหน้า ฯลฯ','ค่าแห่งลิขสิทธิ์ ฯลฯ','ดอกเบี้ย','เงินปันผล เงินส่วนแบ่งกำไร ฯลฯ'])
