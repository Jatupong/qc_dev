...
class ChequeAdvanceConfirmOrder(models.TransientModel):
    ...
    destination_account_id = fields.Many2one('account.account', string='Source/Destination Account')
    ...
    def confirm_order(self):
        ...
        if self.is_single_validate:
            if cheque_ids and cheque_ids[0].type == 'rec':
                print ('rec')
                cheque_ids.action_single_validate(self.validate_date,self.destination_account_id)
            else:
                print ('Payment')
                cheque_ids.action_single_validate_payment(self.validate_date, self.destination_account_id)
        else:
...
class account_cheque_statement(models.Model):
    ...
    #for cheque payment
    def action_single_validate_payment(self,validate_date,destination_account_id):
        print ("action_validate payment")
        total_debit = 0
        all_label = ""
        all_ref = ""
        if not validate_date:
        ...
        for cheque in self:
            ...
            for move_line in move_line_vals:
                if not total_debit and move_line['credit']:
                    all_label += move_line['name'] + ','
                    if move_line['ref']:
                        all_ref += move_line['ref'] + ','
                    total_debit += move_line['credit']
                    move_line['account_id'] = destination_account_id.id
                elif total_debit and move_line['credit']:
                    all_label += move_line['name'] + ','
                    if move_line['ref']:
                        all_ref += move_line['ref'] + ','
                    total_debit += move_line['credit']
        ...
        print ('--ALL MOVE')
        print (all_move_line_vals)
        #####update credit value#### ipdate 23/08/2021
        for move_line in all_move_line_vals:
            if move_line['credit']:
                move_line['credit'] = total_debit
                move_line['name'] = all_label
                move_line['ref'] = all_ref
        #####update credit value#### ipdate 23/08/2021
        move_line_vals = all_move_line_vals
