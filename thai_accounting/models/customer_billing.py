...
class CustomerBilling(models.Model):
    ...
    def _default_sequence_billing(self):
        model = self.env.context.get('move_type')
        print('new',model)
        # return self.env['ir.sequence'].search([('name', '=', 'Customer Billing')])
        if model == 'in_invoice':
            return self.env['ir.sequence'].search([('name', '=', 'Supplier Billing')])
        elif model == 'out_invoice':
            return self.env['ir.sequence'].search([('name', '=', 'Customer Billing')])
    ...
    sequence_billing = fields.Many2one('ir.sequence', string="Sequence",default=_default_sequence_billing)
